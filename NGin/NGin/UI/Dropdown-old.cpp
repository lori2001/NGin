...
namespace NGin
{
	namespace UI
	{
		...
		void Dropdown::handleInput(const sf::Event& event, sf::Sound& pressbutton)
		{
			...
			if (event.mouseButton.button == sf::Mouse::Left && !isInactive)
			{
				...
				if (event.type == sf::Event::MouseButtonPressed && elementSelected[0])
				{
					...
					//drop down
					sprite.setTextureRect(sf::IntRect((int)size.x, 0, (int)size.x, int(size.y * nrofElem)));
					sprite.setSize(sf::Vector2f(size.x, size.y * nrofElem));
			...
			{
				for (int i = 1; i < nrofElem; i++) // for every element
				{
		...
		void Dropdown::draw(sf::RenderTarget & target, sf::RenderStates states) const
		{
			...
			if (isActive[0]) // if dropped down
			{
				for (int i = 1; i < nrofElem; i++) // for each element
				{
		...
		//void Dropdown::setnrofElem(const unsigned int & nrofElem)
		//{
