...
namespace NGin {
	namespace UI
	{
		...
		{
		public:
			Dropdown(const int nrofElem, const sf::Vector2f &size)
			{
				this->size = size;
				this->nrofElem = nrofElem;

				abovetext.setFillColor(sf::Color(98, 144, 219));

				sprite.setSize(size);
				sprite.setOrigin(sf::Vector2f(float(size.x / 2), float(size.y / 2)));
				sprite.setOutlineColor(sf::Color(211, 95, 82));
			...
			Dropdown(const int nrofElem) : Dropdown(nrofElem, sf::Vector2f( 277,40 ))
			{
				//default consturctor inherits from constructor with parameter size but uses default values
			}
			Dropdown(const int nrofElem, const bool isStatic) : Dropdown(nrofElem)
			{
				this->isStatic = isStatic;
			}
			Dropdown(const int nrofElem, const std::string &text) : Dropdown(nrofElem)
			{
				setAbovetext(text);
			}
			...
			int getNrOfElements() const { return nrofElem; }
		...
		private:
			...
			int nrofElem; // holds the number of elements in the dropdown
			...
			std::vector<sf::Text> droptext; // text in dropdown
			std::vector<sf::RectangleShape> dropcolor; // used only if you want colors instead of texts
